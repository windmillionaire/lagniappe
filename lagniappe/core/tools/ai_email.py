...
# @features ai-email
# @dimensions sender-auth dmarc alignment telemetry
...
# @features ai-email
# @dimensions provider-adapter authorization retrieval attachments outbound-email
...
def _preflight_submission(message, tool, user, config):
    ...
    required_access = AI.ASK if tool == "ask" else AI.CREATE
...
def process_resend_email(event, event_id, config, digest_secret, *, client=None):
    ...
    claim = database.claim_ai_email_event(digest, lease_token, now)
    ...
    try:
        ...
        database.release_ai_email_event(
            digest, lease_token, datetime.now(timezone.utc)
        )
        raise
    finally:
        if terminal_state:
            database.finish_ai_email_event(
                digest,
                lease_token,
                terminal_state,
