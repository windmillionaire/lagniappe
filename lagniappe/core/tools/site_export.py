...
# @testable false
# @covered-by lagniappe/core/tools/site_export.py::SiteExportBuilder.build
...
def create_export_record(now=None):
    ...
    export_id = utility.short_uuid()
    prefix = site_export_prefix(export_id, now=now)
    return database.create_site_export(
        {
            "id": export_id,
...
def build_site_export(export_id):
    ...
    record = database.site_export(export_id)
    if not record:
...
class SiteExportBuilder:
    ...
    def _collect_categories(self):
        self.categories = list(_load_iter(database.get.all_categories(), related=True))
        try:
    ...
    def _collect_projects(self):
        self.projects = list(_load_iter(database.get.all_projects(), related=True))
    ...
    def _collect_pages(self):
        pages = [
            page
            for page in _load_iter(database.get.all_pages(), related=True)
    ...
    def _collect_tasks(self):
        tasks = [
            ...
            for task in _load_iter(
                Query(KINDS.instances).filter(Filter().eq("type", "task")).fetch_iter(),
                related=True,
            )
    ...
    def _collect_files(self):
        files = [
            file
            for file in _load_iter(database.get.all_files(), related=False)
    ...
    def _assign_paths(self):
        ...
        for form in self.forms.values():
            form_hash = form.hash or utility.short_hash(form.urlsafe_key)
    ...
    def _register_file(self, file):
        ...
        destination = f"files/{file.hash or utility.short_hash(file.urlsafe_key)}/{filename}"
    ...
    def _entity_slug(self, entity, kind):
        fallback_hash = getattr(entity, "hash", None) or utility.short_hash(entity.urlsafe_key)
    ...
    def _rewrite_asset_url(self, src, entity, current_path):
        ...
        destination = self._copy_named_asset(
            source,
            name,
            f"assets/{getattr(source, 'hash', None) or utility.short_hash(key)}",
        )
        return relative_href(current_path, destination) if destination else src
    ...
    def _copy_public_storage_asset(self, parsed_path):
        ...
        destination = _unique_asset_path(
            f"assets/public/{utility.short_hash(source_path)}-{filename}",
            self._asset_destinations,
        )
    ...
    def _task_anchor(self, task):
        return f"task-{task.hash or utility.short_hash(task.urlsafe_key)}"


# @testable false
# @covered-by lagniappe/core/tools/site_export.py::site_export_prefix
...
# @testable false
# @covered-by lagniappe/core/tools/site_export.py::SiteExportBuilder
