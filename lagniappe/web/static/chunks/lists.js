...
import { BaseList } from './baseList.js?v=b78fefcd';
import { l as localStore } from './storage.js?v=b78fefcd';
...
class ToolReportList extends BaseList {
	...
	async _openDelete(trigger) {
		...
		try {
			const { Modal } = await import('./modal.js?v=b78fefcd');
	...
	async _deleteExecuted(keys, route, modal) {
		...
		this._renderFilters();
		try {
			const { request } = await import('./foundation.js?v=b78fefcd').then(function (n) { return n.o; });
