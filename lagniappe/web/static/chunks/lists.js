...
import { localStore } from './storage.js?v=b43106f0';
...
class ToolReportList extends BaseList {
	...
	async _openDelete(trigger) {
		...
		try {
			const { Modal } = await import('./modal.js?v=b43106f0');
	...
	async _deleteExecuted(keys, route, modal) {
		...
		this._renderFilters();
		try {
			const { request } = await import('./foundation.js?v=b43106f0').then(function (n) { return n.k; });
