...
import { l as localStore } from './storage.js?v=b24ca4c6';
...
class ToolReportList extends BaseList {
	...
	async _openDelete(trigger) {
		...
		try {
			const { Modal } = await import('./modal.js?v=b24ca4c6');
	...
	async _deleteExecuted(keys, route, modal) {
		...
		this._renderFilters();
		try {
			const { request } = await import('./foundation.js?v=b24ca4c6').then(function (n) { return n.n; });
