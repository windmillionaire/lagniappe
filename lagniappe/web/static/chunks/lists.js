...
import { l as localStore } from './storage.js?v=be0bc88b';
...
class ToolReportList extends BaseList {
	...
	async _openDelete(trigger) {
		...
		try {
			const { Modal } = await import('./modal.js?v=be0bc88b');
	...
	async _deleteExecuted(keys, route, modal) {
		...
		this._renderFilters();
		try {
			const { request } = await import('./foundation.js?v=be0bc88b').then(function (n) { return n.n; });
