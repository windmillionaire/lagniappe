...
import { BaseList } from './baseList.js?v=b41d0cfc';
import { l as localStore } from './storage.js?v=b41d0cfc';
import { w as withTransition } from './foundation.js?v=b41d0cfc';
...
class ToolReportList extends BaseList {
	...
	async _openDelete(trigger) {
		...
		try {
			const { Modal } = await import('./modal.js?v=b41d0cfc');
	...
	async _deleteExecuted(keys, route, modal) {
		...
		this._renderFilters();
		try {
			const { request } = await import('./foundation.js?v=b41d0cfc').then(function (n) { return n.o; });
