...
import { localStore } from './storage.js?v=b1aeef6f';
...
class ToolReportList extends BaseList {
	...
	async _openDelete(trigger) {
		...
		try {
			const { Modal } = await import('./modal.js?v=b1aeef6f');
	...
	async _deleteExecuted(keys, route, modal) {
		...
		this._renderFilters();
		try {
			const { request } = await import('./foundation.js?v=b1aeef6f').then(function (n) { return n.k; });
