...
import { c as connectivity } from './chunks/connectivity.js?v=b4fe5366';
import { e as applyNotificationStateHeader } from './chunks/foundation.js?v=b4fe5366';
import { i as installUpstreamUnavailableBanner, c as connectivityMessage } from './chunks/upstreamUnavailable.js?v=b4fe5366';
...
async function onError(event) {
	...
	} = await import('./chunks/foundation.js?v=b4fe5366').then(function (n) { return n.l; });
...
const getView = async () => {
	...
	__activeView = (async () => {
		...
		if (!viewModule) {
			const { captureError } = await import('./chunks/foundation.js?v=b4fe5366').then(function (n) { return n.l; });
			captureError(
...
async function startAnalytics() {
	const { analytics } = await import('./chunks/analytics.js?v=b4fe5366');
	analytics.view();
}
...
function startServiceWorker() {
	...
	navigator.serviceWorker.register("/sw.js").catch(async (error) => {
		const { captureNetworkError } = await import('./chunks/foundation.js?v=b4fe5366').then(function (n) { return n.l; });
	...
	navigator.serviceWorker.addEventListener("controllerchange", async () => {
		...
		const { clearRecentSearchResults } = await import('./chunks/foundation.js?v=b4fe5366').then(function (n) { return n.n; });
...
async function startAuthenticatedLifecycle() {
	...
	const [{ initializeLogoutForms }, { updateUserData }] = await Promise.all([
		import('./chunks/logout.js?v=b4fe5366'),
		import('./chunks/user2.js?v=b4fe5366'),
	]);
