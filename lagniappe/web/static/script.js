...
import { c as connectivity } from './chunks/connectivity.js?v=bdc1ce7d';
import { b as applyNotificationStateHeader } from './chunks/foundation.js?v=bdc1ce7d';
import { i as installUpstreamUnavailableBanner, c as connectivityMessage } from './chunks/upstreamUnavailable.js?v=bdc1ce7d';
...
async function onError(event) {
	...
	} = await import('./chunks/foundation.js?v=bdc1ce7d').then(function (n) { return n.j; });
...
const getView = async () => {
	...
	__activeView = (async () => {
		...
		if (!viewModule) {
			const { captureError } = await import('./chunks/foundation.js?v=bdc1ce7d').then(function (n) { return n.j; });
			captureError(
...
async function startAnalytics() {
	const { analytics } = await import('./chunks/analytics.js?v=bdc1ce7d');
	analytics.view();
}
...
function startServiceWorker() {
	...
	navigator.serviceWorker.register("/sw.js").catch(async (error) => {
		const { captureNetworkError } = await import('./chunks/foundation.js?v=bdc1ce7d').then(function (n) { return n.j; });
	...
	navigator.serviceWorker.addEventListener("controllerchange", async () => {
		...
		const { clearRecentSearchResults } = await import('./chunks/storage.js?v=bdc1ce7d');
...
async function startAuthenticatedLifecycle() {
	...
	const [{ initializeLogoutForms }, { updateUserData }] = await Promise.all([
		import('./chunks/logout.js?v=bdc1ce7d'),
		import('./chunks/user2.js?v=bdc1ce7d'),
	]);
