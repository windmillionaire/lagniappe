...
import { B as BROWSER_PROTOCOL, c as connectivity } from './chunks/connectivity.js?v=be1b1fb2';
...
async function onError(event) {
	...
	} = await import('./chunks/errors.js?v=be1b1fb2');
...
const getView = async () => {
	...
	__activeView = (async () => {
		...
		if (!viewModule) {
			const { captureError } = await import('./chunks/errors.js?v=be1b1fb2');
			captureError(
...
async function startAnalytics() {
	const { analytics } = await import('./chunks/analytics.js?v=be1b1fb2');
	analytics.view();
}
...
function startServiceWorker() {
	...
	navigator.serviceWorker.register("/sw.js").catch(async (error) => {
		const { captureNetworkError } = await import('./chunks/errors.js?v=be1b1fb2');
	...
	navigator.serviceWorker.addEventListener("controllerchange", async () => {
		...
		const { clearRecentSearchResults } = await import('./chunks/utilities.js?v=be1b1fb2');
...
async function startAuthenticatedLifecycle() {
	...
	const [{ initializeLogoutForms }, { updateUserData }] = await Promise.all([
		import('./chunks/logout.js?v=be1b1fb2'),
		import('./chunks/user.js?v=be1b1fb2'),
	]);
