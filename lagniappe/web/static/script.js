...
import { B as BROWSER_PROTOCOL, c as connectivity } from './chunks/connectivity.js?v=bc116afe';
import { e as applyNotificationStateHeader } from './chunks/foundation.js?v=bc116afe';
...
async function onError(event) {
	...
	} = await import('./chunks/foundation.js?v=bc116afe').then(function (n) { return n.l; });
...
const getView = async () => {
	...
	__activeView = (async () => {
		...
		if (!viewModule) {
			const { captureError } = await import('./chunks/foundation.js?v=bc116afe').then(function (n) { return n.l; });
			captureError(
...
async function startAnalytics() {
	const { analytics } = await import('./chunks/analytics.js?v=bc116afe');
	analytics.view();
}
...
function startServiceWorker() {
	...
	navigator.serviceWorker.register("/sw.js").catch(async (error) => {
		const { captureNetworkError } = await import('./chunks/foundation.js?v=bc116afe').then(function (n) { return n.l; });
	...
	navigator.serviceWorker.addEventListener("controllerchange", async () => {
		...
		const { clearRecentSearchResults } = await import('./chunks/foundation.js?v=bc116afe').then(function (n) { return n.n; });
...
async function startAuthenticatedLifecycle() {
	...
	const [{ initializeLogoutForms }, { updateUserData }] = await Promise.all([
		import('./chunks/logout.js?v=bc116afe'),
		import('./chunks/user2.js?v=bc116afe'),
	]);
