...
import { c as connectivity } from './chunks/connectivity.js?v=b564d2b9';
import { e as applyNotificationStateHeader } from './chunks/foundation.js?v=b564d2b9';
import { i as installUpstreamUnavailableBanner, c as connectivityMessage } from './chunks/upstreamUnavailable.js?v=b564d2b9';
...
async function onError(event) {
	...
	} = await import('./chunks/foundation.js?v=b564d2b9').then(function (n) { return n.k; });
...
const getView = async () => {
	...
	__activeView = (async () => {
		...
		if (!viewModule) {
			const { captureError } = await import('./chunks/foundation.js?v=b564d2b9').then(function (n) { return n.k; });
			captureError(
...
async function startAnalytics() {
	const { analytics } = await import('./chunks/analytics.js?v=b564d2b9');
	analytics.view();
}
...
function startServiceWorker() {
	...
	navigator.serviceWorker.register("/sw.js").catch(async (error) => {
		const { captureNetworkError } = await import('./chunks/foundation.js?v=b564d2b9').then(function (n) { return n.k; });
	...
	navigator.serviceWorker.addEventListener("controllerchange", async () => {
		...
		const { clearRecentSearchResults } = await import('./chunks/foundation.js?v=b564d2b9').then(function (n) { return n.l; });
...
async function startAuthenticatedLifecycle() {
	...
	const [{ initializeLogoutForms }, { updateUserData }] = await Promise.all([
		import('./chunks/logout.js?v=b564d2b9'),
		import('./chunks/user2.js?v=b564d2b9'),
	]);
