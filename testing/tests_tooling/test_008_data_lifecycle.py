...
# @pairs data-lifecycle:restore data-lifecycle:rollback data-lifecycle:confirmation data-lifecycle:resume data-lifecycle:cutover data-lifecycle:queue-purge-audit data-lifecycle:remote-journal
def _legacy_named_restore_and_rollback_are_confirmed_resumable_state_machines(
    ...
    (tmp_path / "index.yaml").write_text("indexes: []\n", encoding="utf-8")
    ...
    class Context:
        ...
        def create_database(self, database, location, *, delete_protection):
            self.calls.append(("create", database, location, delete_protection))
            return {"name": "create-operation"}
        ...
        def indexes_ready(self, database):
            return ("indexes", database) in self.calls

        def deploy_indexes(self, database, _path):
            self.calls.append(("indexes", database))
        ...
        def run_runtime_action(self, action, database):
            ...
            return {"status": "current", "rebuilt": action == "rebuild-cache"}

        def version_exists(self, version):
            return version in self.versions

        def deploy_application_version(self, version, database):
            self.calls.append(("deploy-candidate", version, database))
        ...
        def persist_database_setting(self, database):
            self.calls.append(("persist", database))
            return "b" * 64
    ...
    context = Context()
    monkeypatch.setattr(restore_module, "restore_plan", lambda *_args, **_kwargs: plan)
    monkeypatch.setattr(
        restore_module,
    ...
    restore_checkpoint = LifecycleCheckpoint(
    ...
    restored = restore_module.restore_backup(
        ...
        confirmation=lambda _prompt: (
            f"RESTORE {PROJECT_ID} {BACKUP_ID} TO target-db"
        ),
    )
    assert restored == plan
    assert restore_checkpoint.load()["status"] == "complete"
    assert ("persist", "target-db") in context.calls
    assert ("resume-queue", "lagniappe-tasks", "us-central1") in context.calls
    calls_after_restore = list(context.calls)
    assert (
        restore_module.restore_backup(
            BACKUP_ID,
            context=context,
    ...
    assert context.calls == calls_after_restore

    restore_checkpoint.path.unlink()
    rollback_checkpoint = LifecycleCheckpoint(
        PROJECT_ID,
        ["restore", "--rollback", "restore-id"],
        state_root=tmp_path / "rollback-state",
    )
    rolled_back = restore_module.rollback_restore(
        "restore-id",
        context=context,
        checkpoint=rollback_checkpoint,
        confirmation=lambda _prompt: (
            f"ROLLBACK {PROJECT_ID} restore-id TO (default)"
        ),
        state_root=tmp_path / "restore-state",
    )
    assert rolled_back["restore_id"] == plan["restore_id"]
    assert rolled_back["target_database"] == plan["target_database"]
    assert rollback_checkpoint.load()["status"] == "complete"
    assert restore_checkpoint.load()["rolled_back"] is True
    assert ("persist", "(default)") in context.calls
    assert ("traffic", {"current": 1.0}, "random") in context.calls
    rollback_record = json.loads(
        context.bucket.blob(
            "lagniappe-data/v2/restores/restore-id/rollback.json"
        ).download_as_text()
    )
    assert rollback_record["active_database_id"] == "(default)"
    assert rollback_record["purged_tasks"]["object_name"].endswith(
        "/rollback-purged-tasks.json"
    )
    restore_journal = json.loads(
        context.bucket.blob(
            "lagniappe-data/v2/restores/restore-id/journal.json"
        ).download_as_text()
    )
    rollback_journal = json.loads(
        context.bucket.blob(
            "lagniappe-data/v2/restores/restore-id/rollback-journal.json"
        ).download_as_text()
    )
    assert restore_journal["state"]["rolled_back"] is True
    assert rollback_journal["state"]["status"] == "complete"
