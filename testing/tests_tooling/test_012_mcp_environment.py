...
def test_uv_bootstrap_manifest_and_managed_context_path():
    ...
    project = tomllib.loads(
        (REPOSITORY_ROOT / "mcp/pyproject.toml").read_text(
            encoding="utf-8"
        )
    )
    lock = tomllib.loads(
        (REPOSITORY_ROOT / "mcp/uv.lock").read_text(
            encoding="utf-8"
        )
    )
...
def test_mcp_containers_isolate_production_source_and_test_dependencies():
    standard_root = REPOSITORY_ROOT / "runner/hosted_e2e_container"
    metadata_copy = (
        "COPY mcp/pyproject.toml "
        "mcp/uv.lock mcp/README.md "
        "mcp/"
    )
    source_copy = "COPY mcp/src mcp/src"

    service_root = REPOSITORY_ROOT / "mcp"
    for container_root in (standard_root, service_root):
        ...
        assert "--locked --group test --no-editable --no-build-isolation --offline" in dockerfile
        assert "ghcr.io/astral-sh/uv:0.12.9@sha256:" in dockerfile
        assert f"{container_root.relative_to(REPOSITORY_ROOT)}/Dockerfile" in cloudbuild
        final = dockerfile.rsplit("\nFROM ", 1)[1]
        assert "pipx" not in final
        assert "COPY --from=uv-runtime /uv" not in final

    root_ignore = (REPOSITORY_ROOT / ".gcloudignore").read_text(encoding="utf-8")
    assert "/mcp/" in root_ignore
    standard = (standard_root / "Dockerfile").read_text(encoding="utf-8")
    assert standard.index(source_copy) < standard.rindex("COPY . .")
    assert "COPY --from=mcp-builder" in standard.rsplit("\nFROM ", 1)[1]
    standard_ignore = (standard_root / "gcloudignore").read_text(encoding="utf-8")
    assert "!/mcp/**" in standard_ignore
    assert "/mcp/.venv/" in standard_ignore
    assert "/mcp/.uv-cache/" in standard_ignore

    service = (service_root / "Dockerfile").read_text(encoding="utf-8")
    builder, final_service = service.rsplit("\nFROM ", 1)
    assert builder.index("--no-build-isolation --offline") < builder.index(
        "--no-default-groups --no-editable --offline --no-build"
    )
    assert '"-m", "lagniappe_mcp.server"' in final_service
    assert "USER 65532:65532" in final_service
    assert [line for line in final_service.splitlines() if line.startswith("COPY ")] == [
        "COPY --from=builder /workspace/mcp/.venv /workspace/mcp/.venv"
    ]
    allowlist = (service_root / "gcloudignore").read_text(encoding="utf-8").splitlines()
    assert "/*" in allowlist and "/mcp/*" in allowlist
    assert {line for line in allowlist if line.startswith("!")} == {
        "!mcp/", "!mcp/pyproject.toml", "!mcp/uv.lock", "!mcp/README.md",
        "!mcp/src/", "!mcp/src/**", "!mcp/Dockerfile", "!mcp/cloudbuild.yaml",
        "!mcp/gcloudignore",
    }
...
# @matrix mcp-package : command-bridge environment-sync isolation pytest-config
