...
def test_parse_byte_range_standard_and_suffix_forms():
    ...
    open_ended = files.parse_byte_range("bytes=95-", 100)
    assert open_ended.start == 95
    assert open_ended.end == 99

    suffix = files.parse_byte_range("bytes=-25", 100)
    assert suffix.start == 75
    assert suffix.end == 99

    oversized_suffix = files.parse_byte_range("bytes=-125", 100)
...
def test_parse_byte_range_rejects_invalid_headers():
    ...
    assert files.parse_byte_range("", 100) is None
    assert files.parse_byte_range("items=0-10", 100) is None
    assert files.parse_byte_range("bytes=0-10,20-30", 100) is None
    assert files.parse_byte_range("bytes=-0", 100) is None
    assert files.parse_byte_range("bytes=-", 100) is None
    assert files.parse_byte_range("bytes=0-a", 100) is None
...
def test_parse_byte_range_raises_for_unsatisfiable_ranges():
    with pytest.raises(files.UnsatisfiableByteRange):
        files.parse_byte_range("bytes=100-120", 100)

    with pytest.raises(files.UnsatisfiableByteRange):
        files.parse_byte_range("bytes=30-20", 100)
