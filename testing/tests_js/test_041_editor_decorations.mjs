...
test("test_selection_highlight_decorations_and_range_mapping", () => {
	...
	editor.commands.setTrackedRange = (key, range) =>
		trackedCommands.setTrackedRange(
			key,
			range,
		)({
			state,
	...
	const decorationSpec = SelectionHighlight.config.addDecorations.call(context);
	...
	assert.deepEqual(commands.getSelectionHighlightRange()({ editor }), {
		from: 3,
		to: 11,
	});
	assert.deepEqual(
...
test("test_named_tracked_ranges_map_independently", () => {
	const schema = getSchema([StarterKit]);
	let state = EditorState.create({
		schema,
...
test("test_remote_change_flash_decorations_map_and_expire", (t) => {
	...
	storage.author = "Ada";
	const context = { storage };
	const decorationSpec = FlashRemoteChanges.config.addDecorations.call(context);
	const decorationUpdates = [];
	const editor = {
		isDestroyed: false,
		commands: {
	...
	assert.equal(
		blockFlashes.filter(({ attrs }) => attrs["data-editor-author"] === "Ada")
			.length,
		1,
	);
	assert.equal(
		new Set(blockFlashes.map(({ attrs }) => attrs["data-decoration-id"])).size,
		1,
	);
