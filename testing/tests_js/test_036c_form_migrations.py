...
assert.match(status.node.textContent, /Save will be available when the update finishes/);
assert.ok(!header.notification.textContent.includes("Cancel"));
status.show({ ...running, status: "failed", error: "Quantity must be a number.",
  failed_entity: { name: "Checklist", kind: "Task", url: "/tasks/checklist" } });
const link = status.node.children[1].children[0];
assert.equal(link.textContent, "Checklist");
assert.equal(link.href, "/tasks/checklist");
