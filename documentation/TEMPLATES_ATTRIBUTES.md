...
| `lp-deferred` | Create/update form target | Marks an online-only async submit. The initial response acknowledges work; status polling or a later server-change event refreshes the destination. | Requires `data-widget`, `data-route`, and `data-destination`. Completion events match the mounted source widget and destination. |
