...
7. Calls `updateUserData()` to sync the user's timezone, except when `<meta name="mode" content="public">` marks a public page. Browser geolocation remains untouched during ordinary startup. Initializing a `LocationBox` starts the separate location update, serialized after any in-flight timezone update, and its first Places query waits for that location update.
...
| `editRevisionPreview.mjs` | Detached form-widget construction shared by revision comparison and review modals. Delegates to the generic widget loader. |
